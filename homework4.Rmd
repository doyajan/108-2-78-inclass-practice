...
str_extract_all(twoNames, "段+兄弟|段+兄")
str_view_all(twoNames, "段+兄弟|段+兄")
class(twoNames)
```
...
context_picked_split <- as.list(context_picked_split)
...
```{r ans23}
str(context_picked_split)
context_picked_unlist <- unlist(context_picked_split)
str(context_picked_unlist)
class(context_picked_unlist)
mention_duan <- str_extract_all(context_picked_unlist,"段譽|段兄弟|段兄|段大哥|段相公|段君|姓段的小子")
str_count(mention_duan)
class(mention_duan)
 mention_duan
```
...
```{r ans24}
mention_duan_unlist <- unlist(mention_duan)
table_duan <- table(mention_duan_unlist)
 table_duan
```
