...
(20*10+20*15+10*15)*2 -> surface_A
 surface_A <- as.integer(surface_A)
 class(surface_A) 
surface_A
```
...
```{r ans12}
#Length x Width x Height (l,w,h)
surface_cube <- function(l,w,h){
  surface <- as.integer(l*w+l*h+w*h)*2
  return(surface)
}
...
`1st_odd_or_even` <-
if(str_detect(Mask$身分證字號[[1]],'^[A-Z]([12][0-9]{7})[1,3,5,7,9]{1}$',negate=T)){print(as.integer(0))} else if(str_detect(Mask$身分證字號[[2]],'^[A-Z]([12][0-9]{7})[1,3,5,7,9]{1}$',negate=F)){print(as.integer(1))}
...
odd_or_even <- function(n){
  if(str_detect(n,'^[A-Z]([12][0-9]{7})[1,3,5,7,9]{1}$',negate=T)){print(as.integer(0))} else
  if(str_detect(n,'^[A-Z]([12][0-9]{7})[1,3,5,7,9]{1}$',negate=F)){print(as.integer(1))}}
...
get_courses <- function(entranceYear, major){
   major1 <- paste0(major,1)  
   pick_year1 <- courseStructure$學年 == entranceYear
   pick_major1 <- stringr::str_detect(courseStructure$應修系級, major1)
  year1 <- courseStructure$科目名稱[pick_year1&pick_major1]
  year1 <-unique(year1)
  major2 <- paste0(major,2)  
   pick_year2 <- courseStructure$學年 == entranceYear+1
   pick_major2 <- stringr::str_detect(courseStructure$應修系級, major2)
  year2 <- courseStructure$科目名稱[pick_year2&pick_major2]
  year2 <-unique(year2)
 major3 <- paste0(major,3)  
   pick_year3 <- courseStructure$學年 == entranceYear+2
   pick_major3 <- stringr::str_detect(courseStructure$應修系級, major3)
  year3 <- courseStructure$科目名稱[pick_year3&pick_major3]
  year3 <-unique(year3)
  major4 <- paste0(major,4)  
   pick_year4 <- courseStructure$學年 == entranceYear+3
   pick_major4 <- stringr::str_detect(courseStructure$應修系級, major4)
  year4 <- courseStructure$科目名稱[pick_year4&pick_major4]
  year4 <-unique(year4)
  majorlist <-(list(c("year1"=year1,"year2"=year2,"year3"=year3,"year4"=year4))
  return(majorlist)
  print(majorlist)
}
class(get_courses)
str(get_courses(100,"經濟系"))
```
