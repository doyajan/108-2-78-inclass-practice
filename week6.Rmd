...
#用名字選
```{r}
listExample[["student"]] #用$或是[[]]都行，但後者要用字串值(引號非反頓點)
listExample$`學期` #還是要用反頓點
```
```{r}
str(listExample$`學期`)
listExample[["student"]]
...
length(newTaipeiCityVaccine) #只執行一行ctrl+enter
names(newTaipeiCityVaccine)
```
```{r}
str(newTaipeiCityVaccine[[1]])
str(newTaipeiCityVaccine$success)
str(newTaipeiCityVaccine[["succes"]])
...
vectorExample
vectorExample[c(1,3)]
vectorExample[c(3,2,1)]#產生重新排列組合
...
str(taipeiNightMarkets[c("寧夏觀光夜市","雙城街夜市")])
```
#str=string字串
```{r}
vectorExample
